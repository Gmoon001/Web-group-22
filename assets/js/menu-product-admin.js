...
function openOrderManageTable()
{
  ...
  <label for="status">TRẠNG THÁI</label>
    <select id="status" class="status-filter">
...
function renderOrder(orderArray) {
  ...
  for (let i = 0; i < orderArray.length; i++) {
    ...
    for (let j = 0; j < product.length; j++) {
      ...
      while (k < orderProductTmp.length) {
        if (orderProductTmp[k].id == product[j].id) {
          soluong++;
          nameTmp = product[j].name;
          if (soluong < 2) productName += nameTmp;
          orderProductTmp.splice(k, 1);
        } else {
          k++;
        }
    ...
    <div class="order-item">
      ...
      <div class="total-price">${formatPrice(orderArray[i].totalPrice)} ₫</div>
      <div class="status">
        <select class="order-status" onchange="updateOrderStatus(${orderArray[i].idOrderForm}, this.value)">
          <option value="pending" ${orderArray[i].status === 'pending' ? 'selected' : ''}>Chưa xử lý</option>
          <option value="confirmed" ${orderArray[i].status === 'confirmed' ? 'selected' : ''}>Đã xác nhận</option>
          <option value="delivered" ${orderArray[i].status === 'delivered' ? 'selected' : ''}>Đã giao thành công</option>
          <option value="canceled" ${orderArray[i].status === 'canceled' ? 'selected' : ''}>Đã hủy</option>
        </select>
      </div>
      <div class="order-product" onclick="renderDetailsOrder(${orderArray[i].idOrderForm}); showOrderDetails()">
        <i class="fa-solid fa-circle-info"></i>
